                                                ...
                                                <tr class="bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-600 transition duration-200">
                                                    ...
                                                    <td class="px-6 py-4 text-sm">
                                                        <div class="flex justify-end space-x-4">
                                                            <!-- Edit Button -->
                                                            <a href="{{ route('academicians.edit', $member->id) }}" class="bg-gradient-to-r from-green-400 to-green-500 text-white py-1 px-3 rounded-md shadow-md text-sm font-medium hover:bg-green-600 transition duration-300">
                                                                Edit
                                                            </a>
                                                            <!-- Delete Button -->
                                                            <form action="{{ route('projectMembers.destroy', $member->pivot->id) }}" method="POST" class="text-sm">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="bg-gradient-to-r from-red-400 to-red-500 text-white py-1 px-3 rounded-md shadow-md font-medium hover:bg-red-600 transition duration-300" onclick="return confirm('Are you sure?')">
                                                                    Delete
                                                                </button>
                                                            </form>
                                                        </div>
